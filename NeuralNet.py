...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        """
        """YOUR CODE"""
        inActsWithBias = [1.0] + inActs
        return self.sigmoid(self.getWeightedSum(inActsWithBias))
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """YOUR CODE"""
        inActsWithBias = [1.0] + inActs
        return self.sigmoidDeriv(self.getWeightedSum(inActsWithBias))
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i, weight in enumerate(self.weights, 0):
        	newWeight = weight + (alpha * inActsWithBias[i] * delta)
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            ...
            for perceptron in layer:
                inputs.append(perceptron.sigmoidActivation(output[len(output)-1]))
        ...
        for example in examples: #for each example
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    error = 0.0
                    for n, perceptron in enumerate(nextLayer):
                    	temp_delta = nextLayer[n].weights[neuronNum + 1] * deltas[0][n]
                    	error += temp_delta
                    delta = gPrime * error
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    for example in examplesTest:
        feedForward = nnet.feedForward(example[0])[-1]
        correct = True
        for n, output in enumerate(feedForward):
            if round(output) != example[1][n]:
                correct = False
        if correct:
            testCorrect += 1
        else:
            testError += 1
        total += 1
    testAccuracy= float(testCorrect / total) #num correct/num total
